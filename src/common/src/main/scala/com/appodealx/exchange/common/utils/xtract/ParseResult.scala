...
sealed trait ParseResult[+A] {
  ...
  private def prependErrors(errs: List[ParseError]): ParseResult[A] = this match {
   ...
   */
  def fold[B](onFailure: List[ParseError] => B)(onSuccess: A => B): B = {
    if (isSuccessful) {
      onSuccess(get)
    } else {
      onFailure(errors)
...
case class ParseSuccess[+A](get: A) extends ParseResult[A] {
  ...
  def errors: List[ParseError] = Nil
 ...
 */
case class PartialParseSuccess[+A](get: A, errors: List[ParseError]) extends ParseResult[A] {
  def isSuccessful = true
}
 ...
 */
case class ParseFailure(errors: List[ParseError]) extends ParseResult[Nothing] {
